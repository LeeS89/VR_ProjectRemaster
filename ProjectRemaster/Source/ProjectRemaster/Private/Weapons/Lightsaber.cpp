...
ALightsaber::ALightsaber()
{
	...
	TraceComp = CreateDefaultSubobject<UWeaponTraceComponent>(TEXT("Trace Component"));
...
void ALightsaber::BeginPlay()
{
	...
	PreviousLocation = StaticMeshComp->GetComponentLocation();
}
...
void ALightsaber::Tick(float DeltaTime)
{
	...
	CurrentLocation = StaticMeshComp->GetComponentLocation();//GetActorLocation();
...
void ALightsaber::UpdateSlicingPlaneRotation()
{
	...
	if (!MovementDirection.IsNearlyZero()) // Ensure the saber is moving
	{
		...
		SlicingPlane->SetRelativeRotation(FinalRotation);
	}
}
